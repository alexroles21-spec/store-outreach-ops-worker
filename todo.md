...
- [x] Expand GitHub discovery fallback across additional permitted public Common Crawl query patterns so the cycle continues toward 84 qualified stores when one index query returns too few candidates. (Page-offset fallback verified; the latest public source returned 15 candidates, so the run correctly recorded below-target completion.)
