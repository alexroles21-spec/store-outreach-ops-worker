...
- [x] Restore a complete all-qualified-stores directory, preserving historical batches such as the 170-store set, while keeping a separate contact queue for both CAPTCHA and non-CAPTCHA leads filtered by the requested niches and regions. (Current repository retains 206 records; report exposes 91 ready routes and 37 protected forms without deleting the stable directory.)
- [x] Restore every eligible store with a public contact route to the send-ready page, including CAPTCHA and non-CAPTCHA records with personalized subject/body, while keeping the complete Stores directory separate. (Superseded by the final mixed queue restoration below.)
- [x] Keep two external pages distinct: mixed Contact Queue with personalized name/niche/subject/body for all eligible stores, and Stores Directory with stable name/niche/support email/store URL records. (Contact Queue now renders all 177 qualified records; Stores remains the complete directory.)
- [x] Enforce requested niche-only reports, exclude locked/unreachable stores from hourly qualified targets with replacement backfill, and keep Stores Directory separate from the complete personalized Contact Queue. (Added password-lock detection, replacement behavior, clean support-email rendering, and verified the two separate reports.)
- [x] Improve cloud worker reliability with a quarter-hour fallback schedule and same-hour idempotency, publish refreshed Pages after report commits, and keep all 177 qualified contact drafts visible. (Schedule runs at 0/15/30/45 minutes; a completed 84-store UTC hour is skipped; Pages is explicitly dispatched after commits.)
- [x] Fix the live GitHub Pages Contact Review artifact so it shows all 177 qualified personalized cards, not only the legacy 37 protected-review subset, while preserving the separate Stores Directory and verifying hourly run visibility. (Actual GitHub repository now publishes 262 current qualified cards from its preserved 302-record dataset; Pages run 33073722627 succeeded.)
- [x] Add copy buttons beside every Contact Queue subject and message, and contain long fields inside responsive cards on mobile and desktop. (Live Pages verified 262 cards, 262 subject-copy buttons, 262 message-copy buttons, and responsive overflow containment.)
- [x] Republish the live Contact Queue so Copy subject and Copy message buttons visibly appear beside the fields on the public page, including mobile layout verification. (Both `contact-queue.html` and `contact-review.html` are live with 262 subject buttons, 262 message buttons, no-cache headers, and responsive styles.)
- [x] Make Contact actions prefer real store contact pages over mailto links, label email-only records honestly, and restrict newly qualified storefronts to English-language signals in the requested regions. (Live queue verified with 262 cards, 147 Contact-page links, 94 email-only links, and English/UK detection tests passing.)
- [x] Enforce English-only storefront signals for newly discovered stores and keep all generated niche labels, subjects, and messages in English across US, Canada, Australia, UK, and EU targets. (Added explicit `lang`/non-English marker checks and UK `.uk`/`.co.uk` recognition.)
- [x] Make the Contact button open only verified http/https Contact pages, never `mailto:` or Messenger, and label email-only records separately. (Live Pages verified 262 cards, 53 browser Contact pages, 94 email-only labels, and zero clickable mailto Send links.)
- [x] Add an Open email action for email-only leads that opens the recipient address with the personalized subject and message, while keeping Contact-page links browser-only. (Live Pages verified 262 cards, 53 Contact pages, 94 personalized Open email actions, and zero mailto Contact-page links.)
- [x] Require both a verified http/https Contact page and a usable support email for Contact Queue eligibility, remove records missing either field from that queue, and preserve only the complete directory history in Stores. (Superseded by the later Contact-primary rule below.)
- [x] Make Contact page availability the primary queue rule, include Contact-only stores with personalized drafts, and keep stores with support email represented in the separate email directory. (Completed by the current Contact-primary restoration.)
- [x] Restore Contact Queue to every non-sent qualified store with a verified Contact page, include Contact-plus-email stores in both pages, and keep Contact-only stores out of the Email Directory. (Live Pages now show 162 Contact cards after the successful 84-target cloud run; Email Directory remains separate.)
- [x] Broaden new discovery to English-language physical-product Shopify/ecommerce niches such as electronics, accessories, women’s and men’s fashion, beauty, home, fitness, and lifestyle; exclude General e-commerce and artwork-only stores while preserving Contact-first eligibility. (Live Pages verified zero Generic e-commerce rows, 103 Contact cards, and 158 priority physical-product directory records.)
- [x] Add bounded live HTTP activity checks with retries, active/dead classification, and evidence before qualifying discovered stores. (Added a bounded second live HTTP recheck; inactive stores are excluded and evidence records both checks.)
