...
export default function Leads() {
  ...
  const manualPreview = trpc.message.preview.useQuery(selected ? { storeName: selected.storeName, niche: selected.niche, storeUrl: selected.storeUrl } : { storeName: "Review", niche: "e-commerce", storeUrl: "https://example.com" }, { enabled: Boolean(selected?.contactFormProtected) });
  ...
  const markSent = trpc.leads.markSent.useMutation({
    onSuccess: () => { toast.success("Manual send recorded"); utils.leads.search.invalidate(); utils.leads.events.invalidate(); utils.dashboard.stats.invalidate(); },
    onError: error => toast.error(error.message),
  });
