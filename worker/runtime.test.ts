...
describe("worker runtime", () => {
  it("clamps the target to 84 and supports safe dry-run configuration", () => {
    expect(createWorkerConfig({ WORKER_INTERVAL_MINUTES: "60", WORKER_TARGET_PER_RUN: "120", WORKER_DRY_RUN: "true" })).toEqual({ intervalMinutes: 60, targetPerRun: 84, dryRun: true });
  });

  it("does not call the discovery cycle in dry-run mode", async () => {
    const cycle = vi.fn();
    const runner = createWorkerRunner({ intervalMinutes: 60, targetPerRun: 84, dryRun: true }, cycle);
  ...
  it("prevents overlapping cycles", async () => {
    ...
    const runner = createWorkerRunner({ intervalMinutes: 60, targetPerRun: 84, dryRun: false }, cycle);
