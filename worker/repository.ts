...
export function renderPages(leads: RepoLead[], runs: Array<Record<string, unknown>>) {
  ...
  const contactReviewPage = shell("Contact queue", `<p>${reviewLeads.length} pending contact lead(s): ${readyLeads.length} ready routes and ${protectedLeads.length} protected forms. Open each route, verify the site permits contact, send manually, then use the remove button and record the sent state in GitHub.</p><h2>Ready contact routes</h2>${readyLeads.map(renderContactCard).join("") || "<p>No ready contact routes are waiting.</p>"}<h2>Protected forms requiring manual review</h2>${protectedLeads.map(renderContactCard).join("") || "<p>No protected forms are waiting for review.</p>"}`);
