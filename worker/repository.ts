...
import { collectGeoCandidates, hasBrowserContactPage, isPriorityNiche, isUsableEmail, qualifyStore, personalizeMessage } from "../server/outreach";
...
export function renderPages(leads: RepoLead[], runs: Array<Record<string, unknown>>) {
  ...
  const storesPage = shell("Verified stores", `<p>${reportLeads.length} priority physical-product records. Fields are ordered as store name, niche, support email, region, store URL, and contact route. Generic e-commerce and artwork-only records are excluded. Download CSV/JSON for phone-friendly follow-up.</p>${table}`);
