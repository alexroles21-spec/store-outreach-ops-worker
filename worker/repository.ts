...
export function renderPages(leads: RepoLead[], runs: Array<Record<string, unknown>>) {
  ...
  const reviewCards = reviewLeads.map(lead => `<article class="review-card" data-host="${esc(lead.normalizedHost)}"><h2>${esc(lead.storeName)}</h2><p><strong>Niche:</strong> ${esc(lead.niche)} · <strong>Region:</strong> ${esc(lead.region)}</p><p><strong>Store:</strong> <a href="${esc(lead.storeUrl)}" target="_blank" rel="noreferrer">${esc(lead.storeUrl)}</a></p><p><strong>Reason:</strong> ${esc(lead.protectionReason ?? "Protected form")}</p><p><a class="send" href="${esc(lead.publicContactRoute ?? lead.storeUrl)}" target="_blank" rel="noreferrer">Open contact form</a> <button class="confirm-send" type="button" onclick="markSentLocally('${esc(lead.normalizedHost)}')">I sent it — remove from queue</button> <a href="https://github.com/alexroles21-spec/store-outreach-ops-worker/actions/workflows/mark-review-sent.yml" target="_blank" rel="noreferrer">Record sent permanently in GitHub</a></p><dl><dt>Sender email</dt><dd>${esc(lead.senderEmail)}</dd><dt>Subject</dt><dd>${esc(lead.subject)}</dd><dt>Message</dt><dd><pre>${esc(lead.body)}</pre></dd></dl></article>`).join("");
