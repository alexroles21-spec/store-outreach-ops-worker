...
export function renderPages(leads: RepoLead[], runs: Array<Record<string, unknown>>) {
  ...
  const emailOnlyLeads = reviewLeads.filter(lead => !lead.contactFormProtected && /^mailto:/i.test(lead.publicContactRoute ?? ""));
  const unavailableLeads = reviewLeads.filter(lead => !lead.publicContactRoute);
  const renderContactCard = (lead: RepoLead) => {
    const emailHref = lead.contactEmail ? `mailto:${lead.contactEmail}?subject=${encodeURIComponent(lead.subject)}&body=${encodeURIComponent(lead.body)}` : undefined;
    const contactAction = /^https?:\/\//i.test(lead.publicContactRoute ?? "") ? `<a class="send" href="${esc(lead.publicContactRoute)}" target="_blank" rel="noreferrer">Open Contact page</a>` : emailHref ? `<a class="send email-send" href="${esc(emailHref)}">Open email</a>` : `<span class="unavailable">No public Contact page found</span>`;
    return `<article class="review-card" data-host="${esc(lead.normalizedHost)}"><h2>${esc(lead.storeName)}</h2><p><strong>Niche:</strong> ${esc(lead.niche)} · <strong>Region:</strong> ${esc(lead.region)}</p><p><strong>Store:</strong> <a href="${esc(lead.storeUrl)}" target="_blank" rel="noreferrer">${esc(lead.storeUrl)}</a></p><p><strong>Route:</strong> ${esc(lead.contactRouteType ?? (lead.contactEmail ? "email" : "contact form"))} · <strong>Status:</strong> ${esc(lead.contactStatus)}</p><p>${contactAction}${/^mailto:/i.test(lead.publicContactRoute ?? "") ? ` <span class="email-only">Email-only — opens your mail app</span>` : ""} <button class="confirm-send" type="button" onclick="markSentLocally('${esc(lead.normalizedHost)}')">I sent it — remove from queue</button> <a href="https://github.com/alexroles21-spec/store-outreach-ops-worker/actions/workflows/mark-review-sent.yml" target="_blank" rel="noreferrer">Record sent permanently in GitHub</a></p><dl><dt>Sender email</dt><dd class="field-value">${esc(lead.senderEmail)}</dd><dt>Subject</dt><dd class="field-row"><span class="field-value">${esc(lead.subject)}</span><button type="button" class="copy-button" onclick="copyField(this.previousElementSibling)">Copy subject</button></dd><dt>Message</dt><dd class="field-row field-row-message"><pre class="field-value">${esc(lead.body)}</pre><button type="button" class="copy-button" onclick="copyField(this.previousElementSibling)">Copy message</button></dd></dl></article>`;
  };
  const reviewCards = [...protectedLeads, ...readyLeads].map(renderContactCard).join("");
  const shell = (title: string, body: string) => `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta http-equiv="Cache-Control" content="no-cache, no-store, must-revalidate"><meta http-equiv="Pragma" content="no-cache"><meta http-equiv="Expires" content="0"><title>${esc(title)}</title><style>body{font:15px system-ui;margin:40px;background:#f7f5ef;color:#263e31}main{max-width:1200px;margin:auto;min-width:0}nav{margin-bottom:24px;overflow-wrap:anywhere}nav a{color:#397457}table{width:100%;border-collapse:collapse;background:white}th,td{padding:12px;border-bottom:1px solid #e8e4db;text-align:left;vertical-align:top}th{font-size:12px;text-transform:uppercase;color:#777}button{border:1px solid #397457;background:white;color:#397457;border-radius:8px;padding:9px 12px;cursor:pointer}a{color:#397457}.review-card{background:white;border:1px solid #e8e4db;border-radius:12px;padding:22px;margin:16px 0}.review-card h2{margin-top:0}.send{display:inline-block;background:#397457;color:white;padding:10px 14px;border-radius:8px;text-decoration:none}.confirm-send{margin-left:8px;background:#fff4d6;border-color:#c38a28;color:#714f0b}.unavailable,.email-only{display:inline-block;color:#8a5a18;background:#fff4d6;padding:10px 14px;border-radius:8px}.review-card dl{display:grid;grid-template-columns:150px minmax(0,1fr);gap:10px;margin-top:20px}.review-card dt{font-weight:700}.review-card dd{margin:0;min-width:0}.field-row{display:flex;align-items:flex-start;gap:8px;min-width:0}.field-value{min-width:0;overflow-wrap:anywhere;word-break:break-word}.field-row .field-value{flex:1}.review-card pre{white-space:pre-wrap;overflow:auto;max-width:100%;box-sizing:border-box;background:#f7f5ef;padding:14px;border-radius:8px}.copy-button{flex:none;white-space:nowrap;padding:7px 9px;font-size:12px}.review-card{overflow:hidden}@media(max-width:640px){body{margin:16px}.review-card{padding:16px}.review-card dl{grid-template-columns:1fr;gap:6px}.field-row{flex-wrap:wrap}.field-row .copy-button{margin-left:0}.field-row-message{display:block}.field-row-message .copy-button{margin-top:8px}}</style></head><body><main>${links}<h1>${esc(title)}</h1>${body}</main><script>async function copyField(element){const text=element?.textContent||'';try{await navigator.clipboard.writeText(text);const button=element?.parentElement?.querySelector('.copy-button');if(button){const original=button.textContent;button.textContent='Copied';setTimeout(()=>button.textContent=original,1200);}}catch{alert('Copy failed. Please select the text manually.');}}function hideLocally(host){localStorage.setItem('hidden:'+host,'1');const card=document.querySelector('[data-host="'+CSS.escape(host)+'"]');if(card)card.remove();}function markSentLocally(host){if(confirm('Confirm that you completed the form and sent this message. The card will be removed from this device.')){hideLocally(host);alert('Removed from this review queue. Use “Record sent permanently in GitHub” for durable no-repeat tracking.');}}document.querySelectorAll('[data-host]').forEach(card=>{if(localStorage.getItem('hidden:'+card.dataset.host))card.remove();});</script></body></html>`;
  ...
  const contactReviewPage = shell("Contact queue", `<p>${reviewLeads.length} qualified lead(s) in the send queue: ${readyLeads.length} Contact pages, ${protectedLeads.length} protected forms, ${emailOnlyLeads.length} email-only records, and ${unavailableLeads.length} with no public Contact page. Every card keeps its personalized subject and message; only verified http/https Contact pages are clickable.</p><h2>Contact pages</h2>${readyLeads.map(renderContactCard).join("") || "<p>No Contact pages are waiting.</p>"}<h2>Protected forms requiring manual review</h2>${protectedLeads.map(renderContactCard).join("") || "<p>No protected forms are waiting for review.</p>"}<h2>Email-only — no Contact page</h2>${emailOnlyLeads.map(renderContactCard).join("") || "<p>No email-only records.</p>"}<h2>No public Contact page found</h2>${unavailableLeads.map(renderContactCard).join("") || "<p>No unavailable contact records.</p>"}`);
